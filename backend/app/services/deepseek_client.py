...
class DeepSeekClient:
    ...
    @staticmethod
    def _thinking_payload(enabled: bool, reasoning_effort: str = "high") -> dict[str, Any]:
    ...
    @staticmethod
    def _json_headers(accept: str = "application/json") -> dict[str, str]:
        return {
    ...
    def generate_answer(self, context: dict[str, Any], system_prompt: str) -> tuple[dict[str, Any], int]:
        ...
        payload = {
            ...
            "max_tokens": max(settings.default_max_completion_tokens, 2400),
            **self._thinking_payload(settings.llm_reasoning_enabled, reasoning_effort="low"),
        }
        started = perf_counter()
        try:
            response = self.client.post(
            ...
            think_content = self._sanitize_reasoning_text(
                self._extract_reasoning_text(message) or self._extract_think_content(content)
            )
            ...
            elapsed_ms = int((perf_counter() - started) * 1000)
            return self._fallback_answer(context, f"DeepSeek 调用失败：{exc}"), elapsed_ms

    def generate_answer_stream(
        self, context: dict[str, Any], system_prompt: str
    ) -> Generator[dict[str, str], None, None]:
        """Stream the user-facing answer text.

        The QA orchestrator now provides a deterministic graph-process summary,
        so stream mode disables model reasoning to reduce first-token latency and
        avoid leaking fragmented reasoning chunks to the UI.
        """
        question_type = str(context.get("question_type") or "entity_explanation")
        qa_route = str(context.get("qa_route") or "")
        stream_system = f"{system_prompt.strip()}\n{stream_output_instructions(question_type, qa_route or None)}"
        stream_context = dict(context)
        # Stream mode renders natural language directly, so remove JSON-oriented
        # answer rules to avoid exposing formatting conflicts in reasoning_content.
        stream_context.pop("answer_rules", None)
        stream_context["rendering_rules"] = [
            "直接输出自然语言回答，不要输出 JSON、字段名或提示词说明。",
            "如需过程说明，只能写图谱检索与证据整理摘要。",
            "不要讨论系统提示词、JSON 字段、输出协议、内部设定或格式冲突。",
            "正式回答禁止使用“图谱未提供”“知识库尚不支持”“知识库未提供”“未命中”“没有检索到”等系统视角措辞；待验证项必须改写成具体的核验、小试或补充信息建议。",
        ]
        payload = {
            "model": settings.llm_model,
            "messages": [
            ...
            "max_tokens": max(settings.default_max_completion_tokens, 2400),
            "stream": True,
            **self._thinking_payload(False, reasoning_effort="low"),
        }
        if not settings.llm_api_key:
        ...
        try:
            with self.client.stream(
                "POST",
                "/chat/completions",
                headers=self._json_headers("text/event-stream"),
                json=payload,
            ) as response:
                response.raise_for_status()
                in_think = False
                ...
                for line in response.iter_lines():
                    ...
                    try:
                        ...
                        if choices:
                            ...
                            reasoning_text = self._sanitize_reasoning_text(self._extract_reasoning_text(delta))
                            if reasoning_text:
            ...
            logger.exception("DeepSeek stream call failed")
            fallback = self._fallback_answer(context, f"DeepSeek 流式调用失败：{exc}")
            fallback_text = fallback.get("conclusion", "")
            yield {"event": "token", "text": fallback_text}
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "3. 最后给出下一步需要补充确认或验证的具体动作。"
                        "所有待验证项必须改写成面向用户的行动建议，不得出现“图谱未提供”“知识库尚不支持”“知识库未提供”“未命中”“没有检索到”等系统视角措辞。"
    ...
    def _extract_reasoning_text(payload: dict[str, Any]) -> str:
        """Extract reasoning text from provider response shapes."""
        parts: list[str] = []
    ...
    def _sanitize_reasoning_text(text: str) -> str:
        ...
        # Fallback to a short neutral summary instead of exposing prompt metadata.
        return "正在结合图谱命中的实体、关系和证据边界整理回答。"
    ...
    def _fallback_answer(self, context: dict[str, Any], error_message: str | None = None) -> dict[str, Any]:
        ...
        if configured and error_message:
            direct = "本次模型调用失败，系统已切换为本地安全兜底：先保留可核验信息，再给出保守判断。"
            cautions = error_message
        else:
            direct = "当前未配置 DeepSeek API Key，系统返回本地安全兜底说明。"
            cautions = "请在后端环境变量中配置 DEEPSEEK_API_KEY（兼容读取 MINIMAX_API_KEY），以启用完整问答生成。"
        conclusion = (
            f"【核心结论】\n{direct}\n\n"
            f"【判断依据】\n可核验的相关实体包括：{', '.join(names) if names else '暂无明确实体'}。\n\n"
            f"【注意事项】\n{cautions}\n\n"
            "【总结建议】\n可以补充更明确的原料、方剂、产品、人群、症状、剂型和风味偏好后重新提问；系统会按证据边界继续判断。"
        )
        return {
    ...
    def _fallback_analysis(self, question: str) -> dict[str, Any]:
        ...
        route = get_qa_route_resolver().resolve(question)
