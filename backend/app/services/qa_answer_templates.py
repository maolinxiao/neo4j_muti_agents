...
PRODUCT_DEVELOPMENT_RULES = [
    "产品研发回答先展开定位、证据、配方、工艺和风险，再以【核心结论】总结，最后用【追问建议】承接下一步信息补充。",
    "产品研发问题必须交付研发方案，不能把正文写成竞品、价格带或市场概览。",
    "产品研发问题必须先检索 KB5 名方原型：核心原料与方剂有直接关系时标注“图谱直接证据”；只有功效、人群或风味相似时只能写“参考原型/系统推导”，不得声称最终产品直接源自该名方。",
    "【名方溯源与借鉴】必须展示参考原型、出处、原方组成、继承逻辑、证据性质和原型匹配度；原型匹配度是研发匹配指标，不是临床有效率。",
    "【配方方案】至少给出一个可执行草案；每味原料直接使用“党参：君：作用说明；建议小试用量Xg/份（低/中/高梯度）”这类短行，并汇总单份基准配方、总克数和建议每日份数。不要重复添加“原料：”前缀，也不要在逐味说明后重复汇总一遍君臣佐使。成品比例尚待验证时，只能标成研发小试起始范围和待验证假设。",
    "【配方调整与替换依据】必须将最终配方相对参考原型的每味原料标注为保留、替换、新增或删除；实际替换必须有 CAN_REPLACE 直接证据，只有功能相近但没有替代边时只能标为新增/删除或待核验，不能伪造替换关系。",
    "替换信息同时展示“KB4原始分（换算为100分制）”和“系统综合可信度（100分制＋高/中/低）”，并列出功效、风味、安全、方剂语境等依据；禁止展示0.xxx原始小数，禁止把可信度写成临床疗效概率。",
    ...
    "核心原料必须先做 KB1/KB7 合规闸门；“非药食同源或未匹配”只表示在完成品种、来源、加工规格和适用法规核验前不能直接按普通食品原料使用，不能据此推断唯一监管路径。",
...
def context_answer_rules(question_type: str, qa_route: str | None = None) -> list[str]:
    ...
    rules = [
        ...
        "不要把回答写成图谱字段搬运。正式回答禁止出现“知识图谱中检索到”“图谱提供了”“图谱未提供”“知识库尚不支持”“知识库未提供”“未命中”“没有检索到”等系统视角措辞。",
        "如果问题中出现多个实体或多个症状，必须整体回答，不能只回答其中一项。",
        "需要补充证据时，统一改写成面向用户的行动建议，例如“建议补充原方剂量”“建议开展目标人群感官小试”“定稿前需核验适用法规”，不能编造。",
        f"conclusion 必须按以下小标题组织，不适用部分可省略：{headers}。",
        "正文 Markdown 只允许使用【】小标题、1. 2. 3. 编号列表和 - 无序列表；禁止使用 **标题**、***标题*** 或多星号伪造层级。",
        "正文不能裸露 final_score、professional_score、flavor_acceptance 等字段名或 0.xxx 原始评分；产品研发/替代问题可把 KB4 分值换算为整数百分制，并同时给出高/中/低和评分依据。",
        f"{evidence_header} 只保留支撑判断的业务依据，不要重复折叠区的图谱证据摘要。",
        "evidence_summary 必须写成“图谱检索与证据整理摘要”，并按“任务分流->核心信息检查->KB路由->风险/合规边界->回答与追问”五步编号；待验证项统一写成下一步核验动作，不使用“未提供/未命中”式描述。",
        ...
        "只要回答涉及药材或药方，必须优先检查并输出 KB3 风味证据、目标人群/体质画像和待验证项；需要补充时写成风味评价、目标人群试食或体质分层的下一步建议。",
    ...
    if question_type in {"formula_relation", "herb_efficacy", "formula_replacement"}:
        rules.append("涉及方剂时必须列出组成、君臣佐使与剂量；剂量尚待确认时逐味写明“剂量建议结合原方出处与专业规范进一步核定”。")
    return rules
...
def stream_output_instructions(question_type: str, qa_route: str | None = None) -> str:
    ...
    instructions = (
        ...
        "必须先给专家判断，再给证据依据；正式回答禁止出现“知识图谱中检索到/图谱提供/图谱未提供/知识库尚不支持/知识库未提供/未命中/没有检索到”等系统视角措辞。\n"
        f"正文必须用小标题分段，优先顺序为：{headers}；不适用的小标题可以省略。\n"
        "正文 Markdown 只允许【】小标题、1. 2. 3. 编号列表和 - 无序列表；禁止使用 **标题**、***标题*** 或多星号层级。\n"
        "正文不得裸露 final_score、professional_score、flavor_acceptance 等字段名或 0.xxx 原始评分；产品研发/替代问题可展示换算后的整数百分制，同时注明高/中/低、证据性质和评分依据。\n"
        f"{evidence_header} 要写成业务判断依据，不要重复节点数、关系数和图谱摘要原句。\n"
        "正文必须体现“先分流、再检查缺少的核心信息、再说明 KB 路由、最后给回答和追问”的业务顺序。\n"
        "涉及药材或药方时，正文必须包含风味、人群/体质适配判断；相关验证尚未完成时，改写为感官小试、目标人群试食或体质分层建议。\n"
        "正文必须包含【追问建议】，用 1-3 个具体问题主动补齐体质、人群、方剂、原料、产品、剂型、风味、成本、合规或验证信息；其中至少一问要询问用户偏好的口味方向，以及是否需要避开苦味、涩感或药味。\n"
        "剂量尚待确认时逐味写“剂量建议结合原方出处与专业规范进一步核定”，不要用“若干”“适量”等模糊表达。\n"
...
def compose_constraints_block(question_type: str, qa_route: str | None = None) -> str:
    ...
    return (
        ...
        "3. 先给专家判断，再给证据支撑；正式回答禁止出现“知识图谱中检索到/图谱提供/图谱未提供/知识库尚不支持/知识库未提供/未命中/没有检索到”等系统视角措辞。\n"
        "4. related_entities 返回对象数组，每项至少包含 id、name、entity_type。\n"
        "5. 待验证项必须转成行动建议，例如核定剂量、补充体质分层、开展风味小试、复核禁忌或合规规则；不要描述系统缺了什么。\n"
        "6. conclusion 里不要出现节点 id 或“证据：”式的摘要写法。\n"
        "7. 问到体质/产品推荐时遵守推荐类规则；问到产品研发或替代时可将 CAN_REPLACE 原始小数换算为整数百分制，并另列系统综合可信度、高/中/低和依据，不裸露 0.xxx 原始分。\n"
        "8. conclusion 必须保留【】小标题分段，禁止把多段合并成一整段。\n"
        "9. 涉及药材或药方时必须包含风味与人群/体质适配判断；相关验证尚未完成时给出具体的感官评价和目标人群验证建议。\n"
