# -*- coding: utf-8 -*-
"""t3(showcase-upgrade) 上线脚本（临时，勿提交）：首页三大版块视觉升级前端上线。
1) 回滚点：cp -r frontend-dist frontend-dist.bak-<ts>
2) 上传 frontend/dist/** -> /www/wwwroot/neo4j-agents/frontend-dist
3) 清理 assets 中未被引用的旧 hash .js/.css（保留清单 = index.html 引用 + 主包动态 import chunk）
4) 目录 755 / 文件 644 权限规范化
5) 校验新 index.html 与上传 manifest，输出线上资源清单
用法: python deploy_t3_showcase_upload.py [--verify-sha]
"""
import hashlib
import json
import os
...
MANIFEST_OUT = r"D:\python_workspace\neo4j_muti_agents\.tmp-showcase-t3\_deploy_manifest_t3.json"
...
def main():
    ...
    if rc != 0:
        print("[FATAL] backup failed:", out.strip(), err[:200])
        sys.exit(3)
    ...
    # 3) 新 index.html 引用 + 主包内动态 chunk 引用（DEPLOY.md 教训：必须一并保留）
    ...
    # 主包 bundle 内的动态 import / 资源引用（assets/xxx.js|css）
    for rel in manifest:
        if rel.startswith("assets/") and rel.endswith(".js"):
            local_abs = os.path.join(LOCAL_DIST, rel.replace("/", os.sep))
            try:
                js = open(local_abs, encoding="utf-8", errors="replace").read()
            except OSError:
                js = ""
            for m in re.findall(r'assets/[A-Za-z0-9_.-]+\.(?:js|css)', js):
                keep.add(m.split("/")[-1])
        elif rel == "index.html":
            local_abs = os.path.join(LOCAL_DIST, "index.html")
            js = open(local_abs, encoding="utf-8", errors="replace").read()
            for m in re.findall(r'assets/[A-Za-z0-9_.-]+\.(?:js|css)', js):
                keep.add(m.split("/")[-1])
    print("[keep set] assets:", sorted(keep))
    rc, out, _ = run_cmd(cli, "ls %s/assets/ 2>/dev/null" % REMOTE_DIST)
    ...
    for ref in refs_after:
        ...
        print("[ref check]", ref, "->", out.strip())
    missing_chunks = []
    for chunk in sorted(keep):
        rc, out, _ = run_cmd(cli, "test -f '%s/assets/%s' && echo OK || echo MISSING" % (REMOTE_DIST, chunk))
        if "MISSING" in out:
            missing_chunks.append(chunk)
    print("[chunk keep check] missing:", missing_chunks if missing_chunks else "none")

    with open(MANIFEST_OUT, "w", encoding="utf-8") as f:
        json.dump({"bak_dir": bak_dir, "refs_before": refs_before,
                   "refs_after": refs_after, "keep_assets": sorted(keep),
